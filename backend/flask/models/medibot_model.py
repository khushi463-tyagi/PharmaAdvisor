...
# # Load your dataset
# data = pd.read_csv('data/final_data.csv', encoding='ISO-8859-1')
...
# # Function to predict disease and return precaution
...
#     return disease, precaution[0] if len(precaution) > 0 else "No precautions found for this disease."
...
# Function to predict multiple diseases and return precautions
def predict_disease_and_precaution(symptoms):
    symptoms_vectorized = vectorizer.transform([symptoms])
    probabilities = model.predict_proba(symptoms_vectorized)[0]
    top_indices = probabilities.argsort()[-3:][::-1]  # Get the top 3 predictions
